...
class Paymentmodes extends AdminController
{
    ...
    public function manage()
    {
        if ($this->input->post()) {
            ...
            if (empty($data['paymentmodeid'])) {
                ...
                $message = $success ? _l('added_successfully', _l('payment_mode')) : '';
            } else {
                $success = $this->payment_modes_model->edit($data);
                $message = $success ? _l('updated_successfully', _l('payment_mode')) : '';
            }
    ...
    // Since version 1.0.1
    // Change to show this mode to the client or not
